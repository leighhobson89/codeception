...
$I->scrollTo('#homepage > div:nth-child('.$sport.') > .ajaxWrapper:nth-child('.$market.') .sport-event-name', 0, -20);
...
$selections=$I->executeJS('return document.querySelectorAll(".event-content-expand .quick-bet-item").length;'); //calculate available selections on selected Event
	...
	if ($selections != 0){ // it is general sport coupon
		...
		$I->amGoingTo('Selected '.$selectionName.' selection at '.$eventName.' event for '.$selectionPrice.' price.');
		...
		if ($selections != 0){ // it is horses racecard
			...
			$I->amGoingTo('Selected '.$selectionName.' horse at '.$eventName.' race for '.$selectionPrice.' price.');
			$I->scrollTo('.bet-selection-wrapper:nth-child('.$selection.')',0,-20);
			...
			$I->amGoingTo('Selected '.$selectionName.' selection at '.$eventName.' outright event for '.$selectionPrice.' price.');
			$I->scrollTo('.bet-selection-wrapper:nth-child('.$selection.') .innertube',0,-20);
